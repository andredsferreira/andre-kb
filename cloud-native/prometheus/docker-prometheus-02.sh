...
cleanup() {
  ...
  docker rm -f prometheus node-exporter alertmanager >/dev/null 2>&1 || true
...
docker run -d --name alertmanager \
  --network monitoring \
  --mount type=bind,source=/home/andre/andre-kb/cloud-native/prometheus/alertmanager.yml,target=/etc/alertmanager/alertmanager.yml \
  -p 9093:9093 \
  prom/alertmanager:latest \
  --config.file=/etc/alertmanager/alertmanager.yml

echo "Prometheus, node exporter and Alertmanager are running. Press Ctrl+C to
stop and clean up."
